...
void AInfantryAIController::OnMoveCompleted(FAIRequestID RequestID, const FPathFollowingResult& Result)
{
	...
	if (bHasMissionObjective && (!BB || !BB->GetValueAsBool(HasThreatKey)))
	{
...
void AInfantryAIController::TryInitializeAI()
{
	...
	if (!bHasMissionObjective)
	{
		if (InitialTarget)
		{
			SetHoldPosition(InitialTarget);
		}
		else
		{
			SetHoldPositionLocation(HomeLocation);
		}
	}
	else
	{
...
float AInfantryAIController::GetCurrentObjectiveAcceptanceRadius() const
{
	switch (CurrentMissionObjective)
	{
	case EInfantryMissionObjective::MoveToBoardingZone:
		return BoardingObjectiveAcceptanceRadiusCm;

	case EInfantryMissionObjective::MoveToShelter:
		return ShelterObjectiveAcceptanceRadiusCm;
...
void AInfantryAIController::SetHoldPosition(AActor* PositionActor)
{
	if (!PositionActor)
	{
		//UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] HoldPosition actor is null | Pawn=%s"), *GetNameSafe(GetPawn()));
		return;
	}

	StartMissionObjective(EInfantryMissionObjective::HoldPosition, PositionActor, PositionActor->GetActorLocation(), false);
}

void AInfantryAIController::SetHoldPositionLocation(FVector PositionLocation)
{
	StartMissionObjective(EInfantryMissionObjective::HoldPosition, nullptr, PositionLocation, false);
}

void AInfantryAIController::BeginMoveToShelter(AActor* ShelterPoint)
{
	if (!ShelterPoint)
	{
		//UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] ShelterPoint is null | Pawn=%s"), *GetNameSafe(GetPawn()));
		return;
	}

	StartMissionObjective(EInfantryMissionObjective::MoveToShelter, ShelterPoint, ShelterPoint->GetActorLocation(), true);
}

void AInfantryAIController::BeginMoveToShelterLocation(FVector ShelterLocation)
{
	StartMissionObjective(EInfantryMissionObjective::MoveToShelter, nullptr, ShelterLocation, true);
}

void AInfantryAIController::BeginMoveToBoardingZone(AActor* BoardingPoint)
{
	if (!BoardingPoint)
	{
		//UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] BoardingPoint is null | Pawn=%s"), *GetNameSafe(GetPawn()));
		return;
	}

	StartMissionObjective(EInfantryMissionObjective::MoveToBoardingZone, BoardingPoint, BoardingPoint->GetActorLocation(), true);
}

void AInfantryAIController::BeginMoveToBoardingZoneLocation(FVector BoardingLocation)
{
	StartMissionObjective(EInfantryMissionObjective::MoveToBoardingZone, nullptr, BoardingLocation, true);
}
...
void AInfantryAIController::BeginMissionObjectiveMoveTo(AActor* ObjectivePoint, bool bRunToObjective)
{
	...
	StartMissionObjective(EInfantryMissionObjective::MoveToObjective, ObjectivePoint, ObjectivePoint->GetActorLocation(), bRunToObjective);
}
...
void AInfantryAIController::ResumeCurrentObjective()
{
	...
	//UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] Resumed | Pawn=%s Type=%d Location=%s"),
	//	*GetNameSafe(GetPawn()),
	//	static_cast<int32>(CurrentMissionObjective),
...
void AInfantryAIController::ClearMissionObjective()
{
	...
	bWaitingAtMissionObjective = false;
	CurrentMissionObjective = EInfantryMissionObjective::None;
...
EInfantryMissionObjective AInfantryAIController::GetCurrentMissionObjective() const
{
	return CurrentMissionObjective;
}

void AInfantryAIController::StartMissionObjective(EInfantryMissionObjective ObjectiveType, AActor* ObjectiveActor, FVector ObjectiveLocation, bool bRunToObjective)
{
	FVector ProjectedLocation = ObjectiveLocation;

	if (TryProjectPointToNavMesh(ObjectiveLocation, ProjectedLocation))
	{
		ObjectiveLocation = ProjectedLocation;
	}

	bHasMissionObjective = true;
	bMissionObjectiveRun = bRunToObjective;
	bWaitingAtMissionObjective = false;
	CurrentMissionObjective = ObjectiveType;
	MissionObjectiveActor = ObjectiveActor;
	MissionObjectiveLocation = ObjectiveLocation;

	if (ObjectiveType == EInfantryMissionObjective::HoldPosition)
	{
		ReturnPostLocation = MissionObjectiveLocation;
		bHasReturnPostLocation = true;
	}

	ApplyMissionObjectiveMovement();

	/*UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] Started | Pawn=%s Type=%d Actor=%s Location=%s Run=%d"),
		*GetNameSafe(GetPawn()),
		static_cast<int32>(CurrentMissionObjective),
...
void AInfantryAIController::ApplyMissionObjectiveMovement()
{
	...
	//UE_LOG(LogInfantryAI, Warning, TEXT("[INF OBJECTIVE] Moving | Pawn=%s Type=%d Target=%s Run=%d"),
	//	*GetNameSafe(ControlledPawn),
	//	static_cast<int32>(CurrentMissionObjective),
	//	*MissionObjectiveLocation.ToString(),
	//	bMissionObjectiveRun ? 1 : 0);
}
